...
```typescript
import { CircuitOpenError, CIMaxPollsExceededError, CITerminalFailureError } from './pr-reviewer';
...
try {
  ...
  if (error instanceof CircuitOpenError) {
    ...
    console.log(`Wait ${reviewer.getCircuitBreaker().getRemainingTimeout()}ms before retry`);
