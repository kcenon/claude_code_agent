...
export class PRReviewerAgent {
  ...
  constructor(config: PRReviewerAgentConfig = {}) {
    this.config = {
      ...
      deleteBranchOnMerge: config.deleteBranchOnMerge ?? DEFAULT_PR_REVIEWER_CONFIG.deleteBranchOnMerge,
  ...
  private async parseImplementationResultFile(filePath: string): Promise<ImplementationResult> {
    try {
      ...
      return parseYaml(content) as ImplementationResult;
    } catch (error) {
...
_Auto-generated by AD-SDLC PR Review Agent_`;
  ...
  private getCheckResults(implResult: ImplementationResult): CheckResults {
    return {
      ciPassed: implResult.verification.testsPassed &&
  ...
  private async makeDecision(
    ...
    switch (reviewStatus) {
      ...
      case 'changes_requested':
        action = 'revise';
        reason = qualityGateResult.failures.length > 0
  ...
  private async persistResult(result: PRReviewResult): Promise<void> {
    ...
    try {
      ...
      const yamlContent = stringifyYaml(result, { indent: 2 });
