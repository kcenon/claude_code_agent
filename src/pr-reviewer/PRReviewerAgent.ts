...
import { GitHubReviewClient } from './GitHubReviewClient.js';
import {
...
export class PRReviewerAgent implements IAgent {
  ...
  constructor(
    config: PRReviewerAgentConfig & EnhancedCIConfig = {},
    ...
    this.githubReviewClient = new GitHubReviewClient({
      projectRoot: this.config.projectRoot,
   ...
   * @returns PRCreator instance used by this agent
   */
   ...
   * @returns MergeDecision instance used by this agent
   */
   ...
   * @param workOrderId - Identifier of the work order to review
   * @param options - Review configuration such as dry-run, force-approve, or CI skip
   * @returns Complete review result including PR details, quality metrics, and decision
   */
   ...
   * @param externalCallback - Optional caller-provided progress handler to wrap
   * @returns Wrapped callback or undefined if no external callback provided
   */
   ...
   * @param resultPath - Absolute path to the implementation result YAML file
   * @param options - Review configuration such as dry-run, force-approve, or CI skip
   * @returns Complete review result including PR details, quality metrics, and decision
   */
   ...
   * @param workOrderId - Identifier used to locate the result YAML in the scratchpad
   * @returns Parsed implementation result from the corresponding YAML file
   */
   ...
   * @returns Deserialized implementation result object
   */
   ...
   * @param implResult - Worker output containing branch info and change details
   * @param _options - Review options (reserved for future use)
   * @returns Existing or newly created pull request metadata
   */
   ...
   * @param branchName - Git branch name to search for an open PR
   * @returns Matching pull request or null if none exists
   */
  private async findExistingPR(branchName: string): Promise<PullRequest | null> {
    try {
      ...
      getLogger().debug('No existing PR found', {
   ...
   * @param implResult - Worker output used to build PR title, body, and metadata
   * @returns PR creation options with title, body, base, and head branch
   */
...
_Auto-generated by AD-SDLC PR Review Agent_`;
   ...
   * @param issueId - Issue identifier in the format ISS-NNN-scope-description
   * @returns Scope segment (e.g., "feature") extracted from the issue ID
   */
   ...
   * @param prNumber - GitHub PR number to monitor for CI status
   */
      ...
      switch (result.reason) {
        case 'circuit_open':
          throw new CircuitOpenError(
            this.circuitBreaker.getStatus().failureCount,
            this.circuitBreaker.getStatus().lastFailureTime ?? Date.now()
          );
   ...
   * @param prNumber - GitHub PR number to query
   * @returns PR metadata including state, status checks, and reviews
   */
   ...
   * @param implResult - Worker output containing verification status for tests, lint, and build
   * @returns Aggregated check results mapped from the implementation verification
   */
   ...
   * @param qualityGateResult - Evaluated quality gate outcome with pass/fail details
   * @param comments - Review comments to scan for critical or major severity
   * @param options - Review options that may force approval
   * @returns Computed review status: approved, changes_requested, or rejected
   */
   ...
   * @param _pullRequest - Pull request context (reserved for future decision logic)
   * @param reviewStatus - Computed review status from quality and comment analysis
   * @param qualityGateResult - Quality gate evaluation containing failures and warnings
   * @param _comments - Review comments (reserved for future decision logic)
   * @param _options - Review options (reserved for future decision logic)
   * @returns Decision with action (merge, revise, reject) and reason
   */
  ...
  /**
   * Submit review to GitHub with multi-file line-level comments
   *
   * Uses GitHubReviewClient to submit review with line-level comments
   * attached directly to the diff, providing better code review UX.
   * @param prNumber - GitHub PR number to submit the review on
   * @param status - Review verdict (approved, changes_requested, or rejected)
   * @param comments - Line-level review comments to attach to the diff
   */
  private async submitReview(
    ...
    try {
      ...
      const lineComments = comments
        .filter((c) => !c.resolved)
        .map((c) =>
          GitHubReviewClient.convertToLineComment(
            c.file,
            c.line,
            c.severity,
            c.comment,
      ...
      // Submit multi-file review with line-level comments
      const result = await this.githubReviewClient.submitMultiFileReview({
        prNumber,
        body,
        event,
      ...
      if (!result.reviewSubmitted) {
        ...
        await this.submitReviewFallback(prNumber, status, comments, body);
      }
    } catch (error) {
  ...
  /**
   * Fallback method using gh CLI when GitHub API fails
   * @param prNumber - GitHub PR number to submit the review on
   * @param status - Review verdict determining the gh CLI event flag
   * @param comments - Unresolved review comments to post as individual PR comments
   * @param body - Review body text summarizing the review outcome
   */
  private async submitReviewFallback(
    prNumber: number,
    status: ReviewStatus,
    comments: readonly ReviewComment[],
    body: string
  ): Promise<void> {
    // Map status to gh CLI event flag
    let event: string;
    switch (status) {
      case 'approved':
        event = '--approve';
    ...
    // Add individual comments as general PR comments (fallback)
    for (const comment of comments.filter((c) => !c.resolved)) {
      await this.addReviewCommentFallback(prNumber, comment);
    }
  }

  /**
   * Fallback: Add a review comment as a general PR comment
   * @param prNumber - GitHub PR number to add the comment on
   * @param comment - Review comment with file, line, severity, and message
   */
  private async addReviewCommentFallback(prNumber: number, comment: ReviewComment): Promise<void> {
    try {
      ...
      getLogger().warn('Failed to add review comment', {
        agent: 'PRReviewerAgent',
        prNumber,
        file: comment.file,
        error: error instanceof Error ? error.message : String(error),
   ...
   * @param severity - Review comment severity (critical, major, minor, suggestion)
   * @returns Unicode emoji character corresponding to the severity
   */
  ...
  /**
   * Get the GitHubReviewClient instance for direct access
   * @returns GitHubReviewClient instance used for submitting reviews
   */
  public getGitHubReviewClient(): GitHubReviewClient {
    return this.githubReviewClient;
  }
   ...
   * @returns Decision object, potentially enriched with merge commit hash and timestamp
   */
      ...
      case 'reject':
        try {
          await this.executeCommand([
            ...
            '--comment',
            'Closing due to critical issues.',
          ]);
        } catch (error) {
   ...
   * @param comments - Review comments used to extract improvement suggestions
   * @param qualityGateResult - Quality gate outcome providing positive notes and warnings
   * @returns Structured feedback with positive notes and areas for improvement
   */
   ...
   * @param result - Complete review result to serialize as YAML and write to disk
   */
   ...
   * @param prNumber - PR number experiencing CI failures
   * @param implResult - Implementation context including branch and changed files
   * @returns Handoff document with failure details for the CI Fix Agent
   */
   ...
   * @param prNumber - PR number to retrieve failed checks for
   * @returns Array of checks with failure or error conclusion
   */
  private async getFailedCIChecks(
    ...
    try {
      ...
      getLogger().warn('Failed to get failing CI checks', {
        agent: 'PRReviewerAgent',
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
  ...
  private async getRepoName(): Promise<string> {
    try {
      ...
      getLogger().debug('Failed to detect repo name', {
        agent: 'PRReviewerAgent',
        error: error instanceof Error ? error.message : String(error),
      });
      return 'unknown/repo';
   ...
   * @param handoff - Handoff document to serialize as YAML and write to disk
   * @returns Absolute path to the persisted handoff YAML file
   */
   ...
   *
   * @param _prNumber - PR number to check
  ...
  /**
   * Get configuration
   * @returns Copy of the fully resolved agent configuration
   */
   ...
   * @returns Shared circuit breaker managing CI polling failure thresholds
   */
   ...
   * @returns IntelligentCIPoller instance handling exponential backoff CI polling
   */
 ...
 * @param config - Optional configuration applied only when creating a new instance
 * @returns Existing or newly created singleton PRReviewerAgent
 */
