 ...
 */
const VALID_BRANCH_PREFIXES = ['feature', 'fix', 'docs', 'test', 'refactor', 'chore', 'hotfix'] as const;
...
export class PRCreator {
  ...
  public async createFromImplementationResult(
    ...
    if (!branchValidation.valid) {
      throw new BranchNamingError(implResult.branch.name, branchValidation.reason ?? 'Invalid branch name');
    }
  ...
  public validateBranchNaming(branchName: string): BranchValidationResult {
    ...
    if (!VALID_BRANCH_PREFIXES.includes(prefix as (typeof VALID_BRANCH_PREFIXES)[number])) {
      return {
        valid: false,
        prefix,
        issueNumber: undefined,
  ...
  public inferLabels(changes: readonly FileChange[], branchName: string): LabelInferenceResult {
    ...
    if (branchValidation.valid && branchValidation.prefix) {
  ...
  private inferLabelsFromPath(filePath: string): string[] {
    ...
    if (filePath.includes('ci/') || filePath.includes('.github/') || filePath.includes('workflow')) {
      labels.push('ci');
    }
  ...
  public generatePRContent(
    ...
    // Generate title
    const issueRef = implResult.githubIssue !== undefined ? `#${String(implResult.githubIssue)}` : implResult.issueId;
  ...
  private generateTitleDescription(implResult: ImplementationResult): string {
    ...
    if (firstCommit) {
      ...
      const message = firstCommit.message.replace(/^(feat|fix|docs|test|refactor|chore|perf)(\([^)]+\))?:\s*/i, '');
...
_Auto-generated by AD-SDLC PR Review Agent_`;
  ...
  private getDraftReason(implResult: ImplementationResult): string {
    ...
    if (implResult.tests.coveragePercentage < this.config.draftThreshold) {
      reasons.push(`coverage (${String(implResult.tests.coveragePercentage)}%) is below threshold (${String(this.config.draftThreshold)}%)`);
    }
