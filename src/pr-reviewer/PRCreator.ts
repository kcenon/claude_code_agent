...
export class PRCreator {
   ...
   * @param implResult - Worker Agent output containing branch, changes, and verification data
   * @returns PR creation result with the pull request data, labels, and draft status
   */
   ...
   * @param branchName - Full branch name to validate (e.g., "feature/ISS-001-add-auth")
   * @returns Validation result with parsed prefix, issue number, and description or failure reason
   */
   ...
   * @param changes - List of file changes to analyze for label inference
   * @param branchName - Branch name whose prefix maps to label categories
   * @returns Inferred labels with human-readable reasons for each inference
   */
   ...
   * @param filePath - Source file path to match against known directory/extension patterns
   * @returns Labels derived from path patterns (e.g., "testing", "documentation", "security")
   */
   ...
   * @param implResult - Implementation result to evaluate for draft eligibility
   * @returns True if coverage is below threshold, verification failed, or status is blocked
   */
   ...
   * @param implResult - Implementation result providing changes, branch, and verification data
   * @param labels - Inferred labels to display in the PR body
   * @param isDraft - Whether the PR will be created as a draft
   * @returns PR creation options with conventional commit title, detailed body, and branch info
   */
   ...
   * @param implResult - Implementation result whose first commit message or issue ID forms the title
   * @returns Capitalized description extracted from the first commit or fallback issue ID
   */
   ...
   * @param implResult - Implementation result providing changes, tests, and verification data
   * @param issueRef - Issue reference string (e.g., "#42" or "ISS-001") for the summary section
   * @param labels - Labels to list in the PR body
   * @param isDraft - Whether to include a draft warning banner at the top
   * @returns Full PR body markdown with summary, changes, testing, verification, and traceability
   */
...
_Auto-generated by AD-SDLC PR Review Agent_`;
   ...
   * @param implResult - Implementation result with issue, work order, and commit references
   * @returns Markdown traceability section linking issue, work order, branch, and commits
   */
   ...
   * @param implResult - Implementation result to inspect for draft-triggering conditions
   * @returns Comma-separated explanation of why the PR was created as a draft
   */
   ...
   * @param issueId - Issue identifier (e.g., "ISS-001-auth") to extract scope from
   * @param description - Optional branch description used as fallback for scope extraction
   * @returns Scope keyword for the conventional commit title (defaults to "feature")
   */
   ...
   * @param prefix - Branch prefix (e.g., "feature", "fix", "docs") to map to a commit type
   * @returns Conventional commit type (e.g., "feat", "fix", "docs") or "feat" as default
   */
   ...
   * @param branchName - Head branch name to search for an open PR
   * @returns Existing pull request if found, or null if no matching PR exists
   */
  private async findExistingPR(branchName: string): Promise<PullRequest | null> {
    try {
      ...
      getLogger().debug('No existing PR found for branch', {
        agent: 'PRCreator',
   ...
   * @param options - PR creation options including title, body, base/head branches, and draft flag
   * @returns Created pull request metadata from the GitHub API response
   */
  private async createPR(options: PRCreateOptions): Promise<PullRequest> {
    try {
      ...
      if (viewResult.exitCode !== 0) {
        throw new PRCreationError(options.head, new Error(viewResult.stderr));
      }
  ...
  /**
   * Add labels to PR
   * @param prNumber - Pull request number to add labels to
   * @param labels - Label names to attach to the pull request
   */
  private async addLabels(prNumber: number, labels: readonly string[]): Promise<void> {
    ...
    try {
      ...
      await this.executeCommand(['pr', 'edit', String(prNumber), ...labelArgs]);
    } catch (error) {
      getLogger().warn('Failed to add labels to PR', {
        agent: 'PRCreator',
        prNumber,
        labels,
        error: error instanceof Error ? error.message : String(error),
  ...
  /**
   * Get configuration
   * @returns Shallow copy of the fully resolved PR creator configuration
   */
