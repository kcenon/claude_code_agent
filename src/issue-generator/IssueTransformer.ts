...
export class IssueTransformer {
  ...
  constructor(options: IssueGeneratorOptions = {}, estimator?: EffortEstimator) {
  ...
  public transformAll(sds: ParsedSDS): readonly GeneratedIssue[] {
    ...
    for (const component of sds.components) {
      ...
      if (this.estimator.shouldDecompose(component, this.options.maxIssueSize)) {
        const subIssues = this.decomposeComponent(component, sds.traceabilityMatrix);
        issues.push(...subIssues);
      } else {
        const issue = this.transformComponent(component, sds.traceabilityMatrix, estimation);
        issues.push(issue);
      }
    }
  ...
  public transformComponent(
    ...
    try {
      ...
      const body = this.generateBody(component, traceability, technical, finalEstimation);
  ...
  private decomposeComponent(
    ...
    if (!suggestions || suggestions.length === 0) {
      ...
      const estimation = this.estimator.estimate(component);
      return [this.transformComponent(component, traceabilityMatrix, estimation)];
    }
    ...
    const parentEstimation: IssueEstimation = {
      ...
      factors: {
        ...
        methodCount: component.interfaces.reduce((sum, i) => sum + i.methods.length, 0),
      },
    };

    const parentTraceability = this.buildTraceability(component, traceabilityMatrix);
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      const subBody = this.generateSubTaskBody(component, suggestion, parentId, parentTraceability);
  ...
  private buildLabels(component: SDSComponent, estimation: IssueEstimation): IssueLabels {
    return {
      ...
      autoGenerated: true,
      phase: this.options.phasePrefix ? `${this.options.phasePrefix}-3` : null,
    };
  }
  ...
  private generateBody(
    ...
    sections.push('This issue implements part of the AD-SDLC automated pipeline.\n');
   ...
   */
  public getComponentToIssueMap(issues: readonly GeneratedIssue[]): Map<string, string> {
