...
export class IssueTransformer {
  ...
  private decomposeComponent(
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      issues.push({
        ...
        traceability: {
          ...parentTraceability,
          sdsComponent: `${component.id} (sub-task ${i + 1})`,
        },
        technical: {
  ...
  private buildLabels(
    ...
    return {
      ...
      priority: component.priority || this.options.defaultPriority,
  ...
  private generateBody(
    ...
    if (this.options.includeTraceability) {
      ...
      if (traceability.srsFeature) {
        sections.push(`| SRS Feature | ${traceability.srsFeature} |`);
      }
      if (traceability.prdRequirement) {
    ...
    if (this.options.includeHints && technical.suggestedApproach.length > 0) {
      ...
      for (let i = 0; i < technical.suggestedApproach.length; i++) {
        sections.push(`${i + 1}. ${technical.suggestedApproach[i]}`);
      }
      sections.push('\n');
    }
  ...
  public getComponentToIssueMap(
    ...
    for (const issue of issues) {
      ...
      if (componentId && !map.has(componentId)) {
