...
// This test reads .ad-sdlc/config/ files which are runtime-generated by `ad-sdlc init`.
// Skip when config files don't exist (e.g., CI without prior init).
const configExists =
  fs.existsSync(path.join(process.cwd(), '.ad-sdlc/config/workflow.yaml')) &&
  fs.existsSync(path.join(process.cwd(), '.ad-sdlc/config/agents.yaml'));

describe.skipIf(!configExists)('Enhancement Pipeline Mode Configuration', () => {
  let workflowConfig: WorkflowConfig;
  let agentsConfig: Record<string, unknown>;

  beforeAll(() => {
    const workflowPath = path.join(process.cwd(), '.ad-sdlc/config/workflow.yaml');
  ...
  describe('Parallel Execution Configuration', () => {
    ...
    it('should have parallel execution stage with implementation and regression_testing substages', () => {
      const stages = workflowConfig.pipeline.modes.enhancement.stages;
      const parallelStage = stages.find((s) => s.name === 'parallel_execution');
    ...
    it('should have document_update stage with sequential PRD/SRS/SDS updaters', () => {
      const stages = workflowConfig.pipeline.modes.enhancement.stages;
      const documentUpdateStage = stages.find((s) => s.name === 'document_update');
      ...
      const prdUpdate = documentUpdateStage?.substages?.find((s) => s.name === 'prd_update');
      expect(prdUpdate?.next).toBe('srs_update');

      const srsUpdate = documentUpdateStage?.substages?.find((s) => s.name === 'srs_update');
  ...
  describe('Agent Input/Output Configuration', () => {
    it('should have correct inputs for impact_analysis stage', () => {
      ...
      const inputStrings = impactStage?.inputs?.map((i) => (typeof i === 'string' ? i : i.type));
    ...
    it('should have correct outputs for analysis substages', () => {
      ...
      const docReading = analysisStage?.substages?.find((s) => s.name === 'document_reading');
      expect(docReading?.outputs?.some((o) => o.includes('current_state.yaml'))).toBe(true);

      const codeAnalysis = analysisStage?.substages?.find((s) => s.name === 'codebase_analysis');
      expect(codeAnalysis?.outputs?.some((o) => o.includes('architecture_overview.yaml'))).toBe(
        true
      );
...
describe('Agents.yaml Enhancement Pipeline Configuration', () => {
  ...
  describe('Data Flow Configuration', () => {
    it('should have data flow from document-reader to impact-analyzer', () => {
      ...
      const flow = dataFlow.find((f) => f.from === 'document-reader' && f.to === 'impact-analyzer');
    ...
    it('should have data flow from impact-analyzer to prd-updater', () => {
      ...
      const flow = dataFlow.find((f) => f.from === 'impact-analyzer' && f.to === 'prd-updater');
      expect(flow).toBeDefined();
    ...
    it('should have data flow from regression-tester to pr-reviewer', () => {
      ...
      const flow = dataFlow.find((f) => f.from === 'regression-tester' && f.to === 'pr-reviewer');
      expect(flow).toBeDefined();
