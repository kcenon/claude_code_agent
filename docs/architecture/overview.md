...
| -------------------------------- | -------- | --------------------------- |
| `@anthropic-ai/claude-agent-sdk` | ^0.3.260 | Agent execution runtime     |
| `commander`                      | ^15.0.0  | CLI framework               |
| `chalk`                          | ^6.0.0   | Terminal output styling     |
| `inquirer`                       | ^14.2.1  | Interactive CLI prompts     |
...
| `@vitest/coverage-v8` | ^5.0.0  | Code coverage            |
...
Stage 5: Threat Modeling and Tech Decisions (Parallel)
    ├── Threat Model Writer → STRIDE/DREAD Threat Model from SDS
    └── Tech Decision Writer → Technology Decision documents from SDS technology stack
