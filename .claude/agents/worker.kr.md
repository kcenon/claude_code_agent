...
1. **언어**: 모든 커밋 메시지는 반드시 영어로 작성
2. **형식**: Conventional Commits 명세 엄격히 준수
3. **속성(Attribution) 정책**:
   - 커밋에 AI 생성 서명을 포함하지 말 것
   - "Generated by Claude" 또는 유사한 속성을 추가하지 말 것
   - "Co-Authored-By: Claude" 또는 유사한 것을 포함하지 말 것
   - 코드 소유권은 커밋한 사람에게 귀속됨
4. **예시**:
   ```
   # 좋은 예
